...
Telegram Bot，通过 AI 管理你的 Gmail 收件箱。基于通用 Agent 平台架构，支持插件化工具扩展、多 LLM 自动 Fallback、子 Agent 委派、配置热重载。
...
- SQLite（[modernc.org/sqlite](https://pkg.go.dev/modernc.org/sqlite)，纯 Go 实现，零 CGO 依赖）
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `DB_PATH` | `./data/gmailbot.db` | SQLite 数据库路径 |
...
| `DASHBOARD_AUTH` | — | Dashboard 认证 Token |
...
1. **AuthCheck** — 验证用户是否已绑定邮箱，未绑定返回提示
